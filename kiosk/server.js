...
// ===== Customer "My orders" (Supabase Auth JWT + PostgREST RLS) =====
// Lives on Vercel Express; the Cloudflare Functions copy is the parallel path.
...
function bearerToken(req) {
    ...
    // Accept classic JWTs (eyJ…) from Supabase Auth sessions.
...
app.get('/api/my-orders', async (req, res) => {
    try {
        ...
        if (orderNum) {
            if (!/^\d{1,10}$/.test(orderNum)) {
                return res.status(400).json({ error: 'Invalid order number' });
            }
            const rows = await supabaseSelect(
                token,
                'orders',
                `select=${MY_ORDER_COLS}&order_num=eq.${encodeURIComponent(orderNum)}&limit=1`
            );
            if (!Array.isArray(rows) || rows.length === 0) {
                return res.status(404).json({ error: 'Order not found' });
            }
            const itemRows = await supabaseSelect(
                token,
                'order_items',
        ...
        const rows = await supabaseSelect(
            token,
            'orders',
            `select=${MY_ORDER_COLS}&order=created_at.desc&limit=50`
        );
        return res.json({
