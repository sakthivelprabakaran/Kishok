...
export const onRequestPost = guard(async ({ request, env }) => {
    ...
    const store = await requireStoreAcceptingOrders(database);
    if (store.error) return json({ error: store.error }, store.statusCode);
    const availability = await requireOrderableProduct(database, body.productType);
    if (availability.error) return json({ error: availability.error }, availability.status);
    ...
    const saved = await database.insert('orders', orderToRow(body));
